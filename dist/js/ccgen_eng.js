...
function darkside(p1, tr) {
    ...
    if (p1 != "" && p1 != null) {
        ...
        for (var jou = 0; jou <= 3; jou++) {
            if (document.console.ccoudatfmt.options[jou].selected) {
                break;
            }
        }
        ...
        for (var k = 1; k <= ccghm; k++) {
            ...
            var cn = chkCard(p);
            for (var i = tr; i >= 1; i--) {
                document.getElementById('lista').value = "Generating...";
            ...
            if (clcd && ccck) {
                ...
                } else if (document.console.eccv.value != 'rnd') var eccv = document.console.eccv.value;
                else var eccv = '';
                if (jou == 0) {
                    var out = out += cdif;
        ...
        $.ajax({
            url: 'api-live/bin.php',
            type: 'GET',
            async: true,
            data: 'lista=' + document.console.ccpN.value + '|' + document.console.emeses.value + '|' + document.console.eyear.value + '|' + document.console.eccv.value,
            beforeSend: function() {
                $("#bininfo").text("");
                $("#bininfo").append("<div class='spinner-grow spinner-grow-sm' role='status'><span class='sr-only'></span></div>");
            },
